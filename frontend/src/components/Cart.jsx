...
//       CalculateGreenPoints(cartItems, quizResponses); // updates localStorage.greenScore
//     }
//   }, [cartItems]);
...
//       if (!currentProduct) {
//         console.warn("❌ Could not find match for cart item:", item.name);
//         return [];
//       }

//       if (currentProduct.green_score >= 60) return [];
...
//                 <RecommendationCard key={i} from={rec.from} to={rec.to} />
//               ))}
//             </div>
//           )}
//         </div>
...
export default function Cart() {
  ...
  useEffect(() => {
    if (cartItems.length > 0) {
      const quizResponses = JSON.parse(localStorage.getItem("quizResponses")) || {};
      CalculateGreenPoints(cartItems, quizResponses);
    }
  }, [cartItems]);
      ...
      {cartItems.length > 0 && (
        <div className="mt-12 bg-white rounded-lg shadow-md p-6">
          ...
          <button
            ...
            className="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded-full font-semibold"
          >
            🛍️ Place Order
          </button>
        </div>
